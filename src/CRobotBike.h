#ifndef _CROBOT_BIKE_H__
#define _CROBOT_BIKE_H__
...
class CRobotBike {
    ...
    CRobotBike(const State &x0 = State(8,0.0));

	~CRobotBike();

	CRobotBike(const CRobotBike &rhs):
		x(rhs.x),
		robotWidth(rhs.robotWidth),robotHeight(rhs.robotHeight),min_a(rhs.min_a),max_a(rhs.max_a),
		min_delta(rhs.min_delta),max_delta(rhs.max_delta),basicShape(rhs.basicShape), distanceDimension(rhs.distanceDimension),
    ...
    void setMinInputValues(const double a, const double delta) { min_a = a; min_delta = delta; }
    void setMaxInputValues(const double a, const double delta) { max_a = a; max_delta = delta; }
	...
	double min_a, max_a, min_delta, max_delta;
	double Iz, kf, kr, lf, lr, m;
	double map_modifier;
