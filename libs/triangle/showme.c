...
double xlo[2][IMAGE_TYPES], ylo[2][IMAGE_TYPES];
double xhi[2][IMAGE_TYPES], yhi[2][IMAGE_TYPES];
...
double *polynodeptr[2], *polyholeptr[2];
int *polyedgeptr[2];
...
int subdomains[2];
int *partpart[2];
double *partcenter[2], *partshift[2];
int adjsubdomains[2];
int *adjptr[2];
...
double *ymax;
{
  ...
  *ptr = (double *) malloc((*nodes + 1) * *dim * sizeof(double));
  if (*ptr == (double *) NULL) {
  ...
  for (i = 0; i < *nodes; i++) {
    ...
    for (j = 0; j < *dim; j++) {
      ...
      (*ptr)[index++] = (double) strtod(stringptr, &stringptr);
    }
  }
  fclose(infile);
  index = *dim;
...
double *ymax;
{
  ...
  *pnodes = (int) strtol (stringptr, &stringptr, 0);
  ...
  if (*pnodes > 0) {
    *nodeptr = (double *) malloc((*pnodes + 1) * *dim * sizeof(double));
    if (*nodeptr == (double *) NULL) {
      printf("  Out of memory.\n");
      return 1;
    }
    index = *dim;
    ...
    for (i = 0; i < *pnodes; i++) {
      ...
      for (j = 0; j < *dim; j++) {
        ...
        (*nodeptr)[index++] = (double) strtod(stringptr, &stringptr);
      }
    }
  }
  stringptr = readline(inputline, infile, fname);
  ...
  *holeptr = (double *) malloc((*holes + 1) * *dim * sizeof(double));
  if (*holeptr == (double *) NULL) {
    printf("  Out of memory.\n");
    free(*nodeptr);
    free(*edgeptr);
    return 1;
  ...
  for (i = *firstnumber; i < *firstnumber + *holes; i++) {
    ...
    for (j = 0; j < *dim; j++) {
      ...
      (*holeptr)[index++] = (double) strtod(stringptr, &stringptr);
    }
  }
  fclose(infile);
  if (*pnodes > 0) {
...
double **normptr;
{
  ...
  *normptr = (double *) malloc(((*edges + 1) << 1) * sizeof(double));
  if (*normptr == (double *) NULL) {
    printf("  Out of memory.\n");
    free(*edgeptr);
    return 1;
  }
  ...
  for (i = firstnumber; i < firstnumber + *edges; i++) {
    ...
    if ((*edgeptr)[index + 1] == -1) {
      ...
      (*normptr)[index] = (double) strtod(stringptr, &stringptr);
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
      ...
      (*normptr)[index + 1] = (double) strtod(stringptr, &stringptr);
    } else {
...
double **partshift;
{
  ...
  *partition = (int *) malloc((elems + 1) * sizeof(int));
  ...
  fclose(infile);
  *partcenter = (double *) malloc(((*parts + 1) << 1) * sizeof(double));
  if (*partcenter == (double *) NULL) {
    printf("Error:  Out of memory.\n");
    free(*partition);
    return 1;
  }
  *partshift = (double *) malloc((*parts << 1) * sizeof(double));
  if (*partshift == (double *) NULL) {
    printf("Error:  Out of memory.\n");
    free(*partition);
    free(*partcenter);
    return 1;
  ...
  for (i = 0; i < *parts; i++) {
    (*partcenter)[i << 1] /= (double) partsize[i];
    (*partcenter)[(i << 1) + 1] /= (double) partsize[i];
    (*partcenter)[*parts << 1] += (*partcenter)[i << 1];
    (*partcenter)[(*parts << 1) + 1] += (*partcenter)[(i << 1) + 1];
  }
  (*partcenter)[*parts << 1] /= (double) *parts;
  (*partcenter)[(*parts << 1) + 1] /= (double) *parts;
  free(partsize);
  return 0;
}
...
double yoffset;
{
  ...
  double *point1, *point2;
  double normx, normy;
  double normmult, normmultx, normmulty;
  double windowxmin, windowymin, windowxmax, windowymax;
...
double ymax;
{
  ...
  xscale = (double) (width - line_width - 4) / (xmax - xmin);
  yscale = (double) (height - line_width - 4) / (ymax - ymin);
  if (xscale > yscale) {
    xscale = yscale;
  } else {
    yscale = xscale;
  }
  xoffset = 0.5 * ((double) width - xscale * (xmax - xmin)) -
            xscale * xmin;
  yoffset = (double) height - 0.5 * ((double) height - yscale * (ymax - ymin)) +
            yscale * ymin;
  yscale = - yscale;
  switch(image) {
    case NODE:
  ...
  for (i = 1; i <= elems; i++) {
    if ((partition != (int *) NULL) && !bw_ps) {
      ...
      fprintf(elefile, "%6.3f %6.3f %6.3f s\n",
              (double) rgb[colorindex].red / 65535.0,
              (double) rgb[colorindex].green / 65535.0,
              (double) rgb[colorindex].blue / 65535.0);
    }
...
int llcornery;
{
  ...
  for (i = 1; i <= edges; i++) {
    ...
    if (edgeptr[index] == -1) {
      ...
      normmultx = 0.0;
      if (normx > 0) {
        windowxmax = ((double) (612 - llcornerx) - xoffset) / xscale;
        normmultx = (windowxmax - point1[0]) / normx;
      } else if (normx < 0) {
        windowxmin = ((double) llcornerx - xoffset) / xscale;
        normmultx = (windowxmin - point1[0]) / normx;
      }
      normmulty = 0.0;
      if (normy > 0) {
        windowymax = ((double) (792 - llcornery) - yoffset) / yscale;
        normmulty = (windowymax - point1[1]) / normy;
      } else if (normy < 0) {
        windowymin = ((double) llcornery - yoffset) / yscale;
        normmulty = (windowymin - point1[1]) / normy;
      }
      if (normmultx == 0.0) {
        normmult = normmulty;
...
int eps;
{
  double xxscale, yyscale, xxoffset, yyoffset;
  ...
  xxscale = (460.0 - (double) line_width) / (xmax - xmin);
  yyscale = (640.0 - (double) line_width) / (ymax - ymin);
  if (xxscale > yyscale) {
    xxscale = yyscale;
    ...
    switch (event.type) {
      case ButtonRelease:
        if (event.xany.window == quitwin) {
          ...
          xptr = ((double) event.xbutton.x - xoffset) / xscale;
          yptr = ((double) event.xbutton.y - yoffset) / yscale;
          if ((current_image == PART) && loaded[PART] && explode) {
